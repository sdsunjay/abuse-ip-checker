...
def _make_results():
    clean = IPResult(ip="8.8.8.8", isp="Google LLC", org="Google", country="US", abuse_score=0, total_reports=0)
    warning = IPResult(ip="1.2.3.4", isp="Bad ISP", org="Bad Org", country="RU", abuse_score=38, total_reports=30)
    critical = IPResult(ip="5.6.7.8", isp="Worst ISP", org="Worst", country="CN",
                        abuse_score=90, total_reports=100, dns_blocklists=["dnsbl.dronebl.org"])
...
def test_format_verbose_contains_details():
    results = [IPResult(
        ip="1.2.3.4", isp="Test ISP", abuse_score=50, total_reports=10,
        dns_blocklists=["dnsbl.dronebl.org"],
        reports=[{"reported_at": "2026-01-01", "comment": "Bad actor", "categories": [14], "reporter_country": "US"}],
    )]
